...
class UIController {
    ...
    async copyDataset() {
        ...
        if (window.motilityController) {
            // Use detailed report (segments)
            if (window.motilityController.generateMotilityReport) {
                let rawReport = window.motilityController.generateMotilityReport();

                if (rawReport) {
                    // Clean up intro text for cleaner CSV
                    motDetalle = rawReport.replace('Se observan trastornos segmentarios de la motilidad parietal, con ', '')
                        .replace('Se observan trastornos segmentarios de la motilidad parietal: ', '')
                        .replace(/\n/g, '')
                        .trim();
                    // If it becomes empty or just "Se observan...", handle it
                    if (motDetalle.length < 5) motDetalle = rawReport.replace(/\n/g, '').trim();
                }

                // Append territory or pattern name regardless of rawReport
                if (window.motilityController.pattern && window.motilityController.pattern !== 'none') {
                    const patternData = (typeof MotilityModel !== 'undefined' && MotilityModel.PATTERNS) ? MotilityModel.PATTERNS[window.motilityController.pattern] : null;
                    if (patternData && patternData.name) {
                        if (motDetalle !== '-') motDetalle += ` - Patrón: ${patternData.name}`;
                        else motDetalle = `Patrón: ${patternData.name}`;
                    }
                } else if (window.motilityController.generateConclusion) {
                    const territoryStr = window.motilityController.generateConclusion();
                    if (territoryStr) {
                        if (motDetalle !== '-') motDetalle += ` - ${territoryStr}`;
                        else motDetalle = territoryStr;
                    }
                }
            }
        }
        ...
        const row = [
            ...
            document.getElementById('prot_posicion')?.value || '-',
            document.getElementById('prot_tipo')?.options[document.getElementById('prot_tipo').selectedIndex]?.text || '-',
            document.getElementById('prot_numero')?.value || '-',
            document.getElementById('prot_vmax')?.value || '-',
            document.getElementById('prot_gm')?.value || '-',
            document.getElementById('prot_dvi_disp')?.textContent || '-',
            document.getElementById('prot_ieoa_disp')?.textContent || '-',
            document.getElementById('prot_insuficiencia')?.options[document.getElementById('prot_insuficiencia').selectedIndex]?.text || '-',
